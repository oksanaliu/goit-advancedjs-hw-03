import{i as f,S as w}from"./assets/vendor-CYmbJjjI.js";(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))o(t);new MutationObserver(t=>{for(const n of t)if(n.type==="childList")for(const s of n.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function i(t){const n={};return t.integrity&&(n.integrity=t.integrity),t.referrerPolicy&&(n.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?n.credentials="include":t.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(t){if(t.ep)return;t.ep=!0;const n=i(t);fetch(t.href,n)}})();async function m(e,r=1,i=12){const n=`https://pixabay.com/api/?key=47684004-d700c1255eaadac249fdd5630&q=${encodeURIComponent(e)}&image_type=photo&orientation=horizontal&safesearch=true&page=${r}&per_page=${i}`;try{const s=await fetch(n);if(!s.ok)throw new Error("Failed to fetch images");const u=await s.json();if(!u.hits||u.hits.length===0)throw new Error("No images found");return u}catch(s){throw console.error("Error fetching images:",s),s}}function p(e){const r=document.querySelector(".gallery"),i=e.map(o=>`
  ...
  <div></div>
  <div></div>
</div>`;document.body.appendChild(h);let d=1,c="",g=new w(".gallery a");b.addEventListener("submit",async e=>{if(e.preventDefault(),c=E.value.trim(),!c){l("Please enter a search term.");return}L(),d=1,y();try{const r=Date.now(),i=await m(c,d),o=Math.max(500-(Date.now()-r),0);setTimeout(()=>{if(!i.hits||i.hits.length===0){l("Sorry, there are no images matching your search query. Please try again!"),a();return}p(i.hits),g.refresh(),a()},o)}catch(r){console.error(r),setTimeout(()=>{v("Sorry, there was an error fetching images. Please try again later."),a()},500)}});window.addEventListener("scroll",async()=>{if(window.innerHeight+window.scrollY>=document.body.offsetHeight-100){d+=1,y();try{const e=Date.now(),r=await m(c,d),i=Math.max(500-(Date.now()-e),0);setTimeout(()=>{if(!r.hits||r.hits.length===0){l("No more images to load."),a();return}p(r.hits),g.refresh(),a()},i)}catch(e){console.error(e),setTimeout(()=>{l("No more images to load."),a()},500)}}});
